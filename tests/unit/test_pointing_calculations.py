import logging
import math
from katpoint import conversion as cc
import pytest
...
def degree_to_degree_minute_seconds(argin: float) -> str:
        ...
        try:
            ...
            dms_str = (
                f"{int(degrees * sign)}:{int(minutes)}:{round(seconds, 2)}"
            )
...
def test_pointing_calculations(ra:str,dec:str,timestamp:str):
    ...
    # ra1Angle= Angle(ra_dec[0],u.deg)
    # ra1_hms = round(ra1Angle.hourangle,3)
    # ra2Angle = Angle(ra,u.hourangle)
    # ra2_hms = round(ra2Angle.hourangle,3)
    # assert ra1_hms == ra2_hms
    # dec_angle1 = round(ra_dec[1],3)
    # dec_angle2 = round(Angle(dec, u.deg).deg,3)
    # assert dec_angle1 == dec_angle2
    # dec_angle2 = Angle(dec,u.deg)
    # dec_val = dec_angle1.arcsec
    # dec_val2 = dec_angle2.arcsec
    # #assert ra_val == ra
    # assert dec_val== dec_val2