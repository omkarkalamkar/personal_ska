...
def configure_command_with_trajectory_and_ie_ce(
    ...
    if is_trajectory_key_present and is_ie_offset:
        collimation_offsets = [
...
def test_partial_configure_with_ie_wo_ie(
    tango_context, group_callback, json_factory, partial_type
):
    """ """
    main_configure = json_factory("dishleafnode_configure")
    partial_configure = json_factory("delta_configure")
    if partial_type == "both_trajectory_ie_ce":
