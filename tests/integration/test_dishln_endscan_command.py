...
@pytest.mark.SKA_mid
def test_endscan_command_timeout(tango_context, group_callback, json_factory):
    endscan_command_timeout(
        tango_context,
        DISH_LEAF_NODE_DEVICE,
        group_callback,
...
def endscan_command_error_propogation(
    tango_context, dishln_name, group_callback, configure_input_str
):
    logger.info(f"{tango_context}")
    dev_factory = DevFactory()
...
@pytest.mark.post_deployment
@pytest.mark.SKA_mid
def test_endscan_command_error_propogation(
    tango_context, group_callback, json_factory
):
    endscan_command_error_propogation(
        tango_context,
        DISH_LEAF_NODE_DEVICE,
        group_callback,
        json_factory("dishleafnode_configure"),
