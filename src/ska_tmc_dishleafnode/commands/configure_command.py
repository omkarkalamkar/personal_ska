...
class Configure(DishLNCommand):
    ...
    def do(self: Configure, argin: str) -> Tuple[ResultCode, str]:
        ...
        try:
            ...
            def _invoke_configure_band_callback(
                ...
                """
                self.logger.info(
                    f"ConfigureBand: {result}, {exception}, "
                    + f"{status}, {progress}"
                )
                if result is None:
                    self.logger.info(
                    ...
                    self.logger.info(
                        f"ConfigureBand failed: {result}, {exception}, "
                        + f"{status}, {progress}"
                    )
            ...
            self.logger.info("ConfigureBand Done ---")
            ...
            if self.component_manager.dishMode != DishMode.STOW:
                ...
                self.logger.info(
                    "Result and Message: %s, %s", result_code, message
                )
                if result_code[0] in [ResultCode.FAILED, ResultCode.REJECTED]:
                    return result_code[0], message[0]
    ...
    def invoke_trackloadstaticoff(
        ...
        def _invoke_trackstaticloadoff_callback(
            ...
            """
            self.logger.info(
                f"TrackStaticLoadOff: {result}, {exception}, "
                + f"{status}, {progress}"
            )
            if result is None:
                self.logger.info(
    ...
    def ensure_dish_in_right_dish_mode(
        ...
        def _invoke_setoperatemode_callback(
            ...
            """
            self.logger.info(
                f"SetOperateMode: {result}, {exception}, "
                + f"{status}, {progress}"
            )
            if result is None:
                self.logger.info(
        ...
        self.logger.info("SetOperateMode Done ---")
    ...
    def invoke_track_command(
        ...
        def _invoke_track_callback(
            ...
            """
            self.logger.info(
                f"Track: {result}, {exception}, " + f"{status}, {progress}"
            )
            if result is None:
                self.logger.info(
                ...
                self.logger.info(
                    f"Track failed: {result}, {exception}, "
                    + f"{status}, {progress}"
                )
